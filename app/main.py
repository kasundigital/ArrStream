...
def scan_library(limit=500):
    ...
    for inst in instances:
        for root in (inst["root_folders"] or "").splitlines():
            if root:
                roots.append((inst, Path(root)))
    if not roots:
        roots = [(None, Path(MEDIA_ROOT))]
    scanned = 0
...
def queue_page(request: Request):
    ...
    with db() as conn:
        jobs = conn.execute("SELECT * FROM jobs WHERE status IN ('queued','running','paused') ORDER BY id DESC").fetchall()
...
def queue_clear(request: Request):
    ...
    with db() as conn:
        conn.execute("DELETE FROM jobs WHERE status IN ('queued','paused','cancelled')")
...
def queue_cancel(job_id: int, request: Request):
    if not require_login(request): return RedirectResponse("/login", status_code=303)
    with db() as conn:
        conn.execute("UPDATE jobs SET status='cancelled',finished_at=CURRENT_TIMESTAMP WHERE id=? AND status IN ('queued','running','paused')", (job_id,))
    return RedirectResponse("/queue", status_code=303)
...
def library_page(request: Request, status: str = "all"):
    ...
    return templates.TemplateResponse(request, "library.html", {"title": "Library", "items": items, "counts": counts, "filter": status})
...
def history_page(request: Request):
    ...
    if guard: return guard
    with db() as conn:
        jobs = conn.execute("SELECT * FROM jobs WHERE status NOT IN ('queued','running','paused') ORDER BY id DESC LIMIT 1000").fetchall()
...
def connections_page(request: Request):
    ...
    rows = []
    with db() as conn:
        instances = conn.execute("SELECT * FROM instances ORDER BY kind,name").fetchall()
    for inst in instances:
...
def diagnostics_page(request: Request):
    ...
    if guard: return guard
    checks = []
    checks.append({"name": "FFmpeg", "ok": bool(ffmpeg_encoders()), "detail": ", ".join(ffmpeg_encoders()) or "Not available"})
    checks.append({"name": "Media root", "ok": Path(MEDIA_ROOT).exists(), "detail": MEDIA_ROOT})
    checks.append({"name": "Config writable", "ok": os.access(CONFIG_DIR, os.W_OK), "detail": str(CONFIG_DIR)})
    checks.append({"name": "GPU access", "ok": bool(detect_gpus()), "detail": ", ".join(g["name"] for g in detect_gpus()) or "No GPU exposed; CPU mode available"})
    with db() as conn:
...
def api_connections():
    with db() as conn:
        rows = conn.execute("SELECT id,kind,name,url,enabled,version,root_folders FROM instances ORDER BY id").fetchall()
...
def api_jobs():
    with db() as conn:
        rows = conn.execute("SELECT * FROM jobs ORDER BY id DESC LIMIT 500").fetchall()
...
def api_readiness():
    with db() as conn:
        r = conn.execute("SELECT COUNT(*) total, SUM(readiness='ready') ready, SUM(readiness='optimize') optimize, SUM(readiness='failed') failed FROM library_items").fetchone()
    return dict(r)
